    ...
    if os.path.exists("./documents"):
        for f in os.listdir("./documents"):
            if os.path.isfile(os.path.join("./documents", f)):
                ...
                staged_files.append({"name": f, "size": size_str, "status": "ready"})
...
def update_config(config: ConfigUpdateRequest):
    ...
    if config.routing_method not in ("semantic", "llm"):
        raise HTTPException(
            status_code=400, detail="Invalid routing_method. Must be 'semantic' or 'llm'."
        )
    if config.reranker_provider not in ("flashrank", "cohere"):
        raise HTTPException(
            status_code=400, detail="Invalid reranker_provider. Must be 'flashrank' or 'cohere'."
        )
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Pipeline reload failed. Check server configuration logs."
        )
...
async def trigger_ingestion(raptor: bool = False):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "An internal server error occurred during document ingestion. "
                "Please check server logs."
            ),
        )
