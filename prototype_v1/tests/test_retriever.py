from src.retriever import Retriever, assemble_answer, ask
...
def test_ftth_query_with_clear_keyword_retrieves_ftth_content():
    ...
    chunks = get_retriever().retrieve("How long does FTTH installation take?", k=3, use_category_filter=True)
...
def test_category_filter_narrows_results_when_category_detected():
    chunks_unfiltered = get_retriever().retrieve("CCTV camera resolution options", k=5, use_category_filter=False)
    chunks_filtered = get_retriever().retrieve("CCTV camera resolution options", k=5, use_category_filter=True)
...
def test_ask_end_to_end_returns_sourced_answer():
    answer = ask("What is the standard BPO support desk operating hours?", retriever=get_retriever())
