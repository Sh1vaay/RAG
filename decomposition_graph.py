...
def create_decomposition_graph(retriever: BaseRetriever, llm: ChatOpenAI):
    ...
    def answer_sub_question(state: DecompositionState) -> Dict[str, Any]:
        ...
        ans_prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert technical assistant. Answer the current sub-question based on the retrieved documents and the context of previously answered sub-questions.\n\n"
    ...
    def generate_final_answer(state: DecompositionState) -> Dict[str, Any]:
        ...
        final_prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert technical assistant. Write a comprehensive final answer to the user's main question based on the resolved sub-questions and answers.\n\n"
                       "--- RESOLVED SUB-QUESTIONS & ANSWERS ---\n"
                       "{sub_qa_text}\n"
                       "--- END RESOLVED CONTEXT ---\n\n"
                       "Synthesize these answers into a cohesive, structured response that directly answers the main question. Citations from source documents are already integrated in the sub-answers context, make sure to preserve them."),
