...
This script evaluates the Aether AI pipeline against the two golden sets from the internship project.
...
def word_overlap_faithfulness(retrieved_text: str, golden_answer: str) -> float:
    ...
    stop = {"the", "a", "an", "is", "are", "to", "of", "for", "and", "in", "on", "at",
            "with", "we", "our", "you", "your", "this", "that", "it", "be", "as"}
...
def main():
    ...
    try:
        ...
        print("Please make sure you have copied the dataset into ./documents and run 'python -m src.ingest'")
        return
    ...
    for r in report_v1["results"]:
        ...
        print(f"  [{flag}] faithfulness={r['faithfulness']:.2f} | Route: {r['route_used']} | {r['question']}")
    ...
    print("\n" + "=" * 70)
    print(f"RESULT: {report_v1['avg_faithfulness']:.2f} -> {report_v2['avg_faithfulness']:.2f} "
