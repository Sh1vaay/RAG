...
class SemanticRouter:
    ...
    def route(self, query: str, threshold: float = 0.40) -> Tuple[str, float]:
        ...
        # Compute Cosine Similarity (dot product of normalized embedding vectors)
...
class RoutingRetriever(BaseRetriever):
    ...
    - 'llm': LLM-based structured classifier using Claude Opus 5.
    """
    ...
    def retrieve_for_route(self, query: str, route: str) -> List[Document]:
        ...
        if route == "hyde":
            ...
            try:
                hypothetical_answer = hyde_chain.invoke({"question": query}).text.strip()
                ...
                # Safety similarity threshold. NOTE: 0.75 was tuned against OpenAI
                # text-embedding-3-small; Cohere embed-english-v3.0 has a different
                # similarity distribution, so re-tune if HyDE fires too often/rarely.
                threshold = 0.60
            ...
            try:
                step_back_q = sb_chain.invoke({"question": query}).text.strip()
            ...
            try:
                sub_q_text = dec_chain.invoke({"question": query}).text.strip()
            ...
            try:
                mq_text = mq_chain.invoke({"question": query}).text.strip()
