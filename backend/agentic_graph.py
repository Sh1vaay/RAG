...
_REFLECT_PROMPT = ChatPromptTemplate.from_messages(
    [
        (
            ...
            "to produce it, decide if the answer contains any claims NOT "
...
def create_agentic_graph(retriever: BaseRetriever, llm: BaseChatModel):
    """
    ...
    llm : BaseChatModel
        The shared local Ollama instance from main.py.
    """
    ...
    def rewrite_query(state: AgenticState) -> dict:
        ...
        try:
            new_q = rewrite_chain.invoke({"question": state["question"]}).text.strip()
        except Exception as exc:
